...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PYMTMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PYMTMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PYMTMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PYMT-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running PYMTMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
